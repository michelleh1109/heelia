...
export const DashboardScreen: React.FC<DashboardScreenProps> = ({ onRestart, onOpenEducation }) => {
  ...
  const rangeDescriptor = selectedRange === 'ALL' ? 'your first scan' : 'this period start';
  const trendBadgeLabel = chartSummary.delta >= 0 ? 'Trending upward' : 'Needs attention';
  ...
  return (
    <ScreenContainer style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        <LinearGradient colors={['#FFEAE5', '#F2E9FF']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.heroCard}>
          <View style={styles.heroLeft}>
            <Text style={styles.heroLabel}>Bone density</Text>
            <Text style={styles.heroValue}>{chartSummary.current.toFixed(2)}</Text>
            <Text style={styles.heroSubhead}>T-score today</Text>
          </View>
          <View style={styles.heroRight}>
            ...
            <View style={styles.heroDeltaRow}>
              <Text style={styles.heroDeltaValue}>{deltaLabel}</Text>
              <Text style={styles.heroDeltaHelper}>vs {rangeDescriptor}</Text>
            </View>
            <Text style={styles.heroRange}>{chartSummary.rangeLabel}</Text>
          </View>
        </LinearGradient>
